#!/bin/bash
#SBATCH --job-name=rest_03_project
...
# =============================================================================
# Projects hcptrt resting-state parcel time series through Friends-trained PCA.
# One array task per subject (all six subjects; rest includes sub-04).
#
# Prerequisites:
#   - rest_00_postproc.sh + rest_02_extract_parcel_ts.sh completed
#   - 04_combined_hdphmm.py (mode: select) completed for this subject
#
# Usage:
#   sbatch script/rest_03_project_rest_pca.sh
...
echo "=========================================="
echo "Resting-State PCA Projection"
echo "=========================================="
...
uv run --project "${PROJECT_DIR}" --no-sync python "${PROJECT_DIR}/script/rest_03_project_rest_pca.py" \
    --sub_id "$SUBJECT_ID" \
