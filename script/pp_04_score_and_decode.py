...
This tests whether Friends-trained brain states produce meaningful state sequences
from audio-only narrative data.

Prerequisites:
    - pp_03_project_pp_pca.py completed (projected PP data + pp_run_ids.json)
    - 04_combined_hdphmm.py (mode: select) completed (Friends model)

Outputs:
    {SCRATCH_DIR}/output/pp_04_decoded/{parcellation}/{sub_id}/
        decoded_states.pkl         - dict: run_id -> np.array(n_trs,) state indices
        fractional_occupancy.pkl   - dict: run_id -> np.array(n_states,)
        pp_ll_summary.json         - Per-run LL, overall LL, baselines
...
def _pp_stimulus_for(long_run_id: str) -> str:
    """Infer petitprince sub-stimulus (FR/EN) from a BIDS run_id."""
    if "task-lppFR" in long_run_id or long_run_id.startswith("lppFR"):
        return "petitprince_fr"
    if "task-lppEN" in long_run_id or long_run_id.startswith("lppEN"):
        return "petitprince_en"
    raise ValueError(
        f"Cannot determine PP language (lppFR/lppEN) for run_id={long_run_id!r}"
    )
...
# Petit Prince language type labels
PP_TYPE_LABELS = {
    'lppFR': 'French (Audio)',
    'lppEN': 'English (Audio)',
}
...
def parse_args():
    parser = argparse.ArgumentParser(
        description='Score and decode Petit Prince data with Friends-trained HMM.',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python script/pp_04_score_and_decode.py --sub_id sub-01
  python script/pp_04_score_and_decode.py --sub_id sub-01 --parcellation atlas-4S456Parcels
        """
    )
...
def main():
    ...
    if args.vt is not None:
        out_dir = os.path.join(SCRATCH_DIR, 'output', 'pp_04_decoded', parc, sub_id, f'vt{args.vt}')
    else:
        out_dir = os.path.join(SCRATCH_DIR, 'output', 'pp_04_decoded', parc, sub_id)
    os.makedirs(out_dir, exist_ok=True)

    # =========================================================================
    # Score and decode each PP run
    # =========================================================================
    ...
    for stype, run_ids in pp_run_ids.items():
        ...
        logger.info(f"Processing {label}: {len(run_ids)} runs")
    ...
    # Overall PP LL (weighted average by n_trs)
    ...
    ll_summary = {
        ...
        'pp_overall_ll_per_sample': float(overall_pp_ll),
        'pp_ll_per_run_mean': run_ll_mean,
        'pp_ll_per_run_std': run_ll_std,
        'pp_ll_per_run_se': run_ll_se,
        'pp_total_trs': int(total_trs),
        'pp_total_runs': total_runs_decoded,
        ...
        'll_gap_friends_minus_pp': float(friends_test_ll - overall_pp_ll),
        'pp_above_baseline': bool(overall_pp_ll > baseline_ll),
    ...
    # 'lppEN_run-NN'; language inferred per-run from the BIDS token) and
    # save decoded states, FO, and run_id_map.json (required by pp_05).
    long_to_short, decoded_states_short, fo_short = canonicalize_and_save_decoded(
        decoded_states, out_dir, _pp_stimulus_for, n_states,
        map_stimulus="petitprince")

    with open(os.path.join(out_dir, 'pp_ll_summary.json'), 'w') as f:
    ...
    print(f"\n{'='*60}")
    print(f"PETIT PRINCE SCORE & DECODE SUMMARY")
    print(f"{'='*60}")
    print(f"Subject:                {sub_id}")
    print(f"Parcellation:           {parc}")
    print(f"PP runs decoded:        {total_runs_decoded}")
    print(f"Total PP TRs:           {total_trs}")
    print(f"Friends test LL/sample: {friends_test_ll:.4f}")
    print(f"PP overall LL/sample:   {overall_pp_ll:.4f}")
    print(f"Baseline LL/sample:     {baseline_ll:.4f}")
    print(f"LL gap (Friends-PP):    {friends_test_ll - overall_pp_ll:.4f}")
    print(f"PP > baseline:          {overall_pp_ll > baseline_ll}")
    print(f"")
    print(f"Per-type LL/sample:")
    for stype, info in per_type_ll.items():
        label = PP_TYPE_LABELS.get(stype, stype)
        ll_val = info['ll_per_sample']
        ll_str = f"{ll_val:.4f}" if ll_val is not None else "N/A"
        print(f"  {label:20s}: {ll_str} ({info['n_runs']} runs, {info['n_trs']} TRs)")
    print(f"")
    ...
    try:
        ...
        # Compact tick labels ('FR-NN' / 'EN-NN') from the canonical short id
        ...
        fig, (ax_ll, ax_st) = plt.subplots(1, 2, figsize=(12, 4))

        # Left: per-run LL dot chart
        ll_vals = [per_run_ll[rid] for rid in sorted_rids]
        ax_ll.scatter(range(n_runs), ll_vals, c=colors,
        ...
        ax_ll.axhline(overall_pp_ll, color='gray', ls='--', lw=1.0,
                      label=f'PP mean LL ({overall_pp_ll:.2f})')
        ax_ll.set_xticks(range(n_runs))
        ax_ll.set_xticklabels(short_labels, rotation=45, ha='right', fontsize=7)
        ax_ll.set_ylabel('LL / sample')
        ax_ll.set_title('Per-Run Log-Likelihood')
        ax_ll.legend(fontsize=7, loc='lower left')
        gap = friends_test_ll - overall_pp_ll
        ...
        ax_st.bar(range(n_runs), states_per_run, color=colors,
        ...
        ax_st.set_xticklabels(short_labels, rotation=45, ha='right', fontsize=7)
        ...
        ax_ll.legend(handles=[
            plt.Line2D([0], [0], color='#4682B4', ls='-', lw=1.2, label=f'Friends test LL'),
            plt.Line2D([0], [0], color='gray', ls='--', lw=1.0, label=f'PP mean LL'),
            Patch(facecolor='#2196F3', label='French'),
            Patch(facecolor='#FF9800', label='English'),
        ], fontsize=7, loc='lower left')

        vt_str = f', vt={args.vt}' if args.vt else ''
        fig.suptitle(f'PP Score & Decode ({sub_id}{vt_str})', fontsize=12)
        fig.tight_layout()
