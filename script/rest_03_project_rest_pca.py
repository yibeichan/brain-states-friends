#!/usr/bin/env python3
"""
rest_03_project_rest_pca.py - Project hcptrt resting-state parcel time series through Friends-trained PCA.

Loads the PCA model fitted on Friends training data (from 04_combined_hdphmm final/)
and projects each resting-state run's parcel time series into the same PCA space.
Computes a PCA transfer diagnostic (variance explained by Friends PCA on rest data).

Resting state is a task-free condition (no stimulus, eyes open, ~15 min, 4-6 runs
per subject across all six subjects). This tests whether the Friends PCA subspace -
trained on audiovisual naturalistic viewing - captures meaningful variance in
spontaneous (unstimulated) brain activity.

Prerequisites:
    - rest_00_postproc.sh completed for restingstate (cleaned CIFTIs)
    - rest_02_extract_parcel_ts.sh completed for restingstate runs
    - 04_combined_hdphmm.py (mode: select) completed for this subject (Friends model)

Outputs:
    {SCRATCH_DIR}/output/rest_03_projected/{parcellation}/{sub_id}/[vt{VT}/]
        {run_id}.npy                  - PCA-projected rest run (n_trs, n_pcs)
        rest_run_ids.json             - Run IDs grouped by stimulus type
...
# Resting-state stimulus type prefix
REST_PREFIXES = {
...
def get_stimulus_type(run_id):
    ...
    Args:
        run_id: Full BIDS run ID (e.g., 'sub-01_task-restingstate_run-1_space-fsLR_den-91k')
                or just the task portion (e.g., 'restingstate_run-1')

    Returns:
        Stimulus type string ('restingstate') or None
    """
...
def discover_rest_runs(parcel_ts_dir):
    """Discover resting-state run files in the parcel time series directory.

    Finds all .npy files with restingstate task prefix, excluding Friends episodes
    (task-s*), Movie10, and other hcptrt (HP/PP) runs sharing the same directory.
    ...
    rest_runs = {stype: [] for stype in REST_PREFIXES}
    ...
    for fpath in all_files:
        ...
        if stype is not None:
            rest_runs[stype].append((run_id, fpath))
...
def compute_pca_transfer_diagnostic_from_stats(pca, n_pcs, stats):
    ...
    # Per-parcel mean shift: rest parcel means vs Friends training means
    rest_parcel_means = ov['parcel_sum'] / ov['count'] if ov['count'] > 0 else np.zeros(n_parcels)
    mean_shift = rest_parcel_means - pca.mean_
    ...
    if ov['count'] > 0:
        ...
        sst_mc = sum_x_sq - ov['count'] * np.dot(rest_parcel_means, rest_parcel_means)

        shift = pca.mean_ - rest_parcel_means
    ...
    diagnostic = {
        ...
        'rest_r2_n_pcs': float(r_squared),
        'rest_r2_n_pcs_mean_corrected': float(r_squared_mc),
        'rest_r2_full_pca': float(r_squared_full),
        'friends_r2_n_pcs': friends_r2_n_pcs,
        'friends_r2_full_pca': friends_r2_full,
        'n_pcs': n_pcs,
        'n_components_total': pca.n_components_,
        'n_rest_trs': int(ov['count']),
        'n_parcels': n_parcels,
...
def parse_args():
    parser = argparse.ArgumentParser(
        description='Project hcptrt resting-state parcel time series through Friends-trained PCA.',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python script/rest_03_project_rest_pca.py --sub_id sub-01
  python script/rest_03_project_rest_pca.py --sub_id sub-01 --parcellation atlas-4S456Parcels
        """
    )
...
def main():
    ...
    if not os.path.isdir(parcel_ts_dir):
        ...
        logger.error("Run rest_00_postproc.sh and rest_02_extract_parcel_ts.sh first.")
        sys.exit(1)
    ...
    # =========================================================================
    # Discover and group rest runs
    # =========================================================================

    rest_runs = discover_rest_runs(parcel_ts_dir)

    total_runs = sum(len(runs) for runs in rest_runs.values())
    if total_runs == 0:
        logger.error(f"No rest runs found in {parcel_ts_dir}")
        logger.error("Expected files matching *task-restingstate*_parcel_avg.npy")
        sys.exit(1)

    logger.info(f"Discovered {total_runs} rest runs:")
    for stype, runs in rest_runs.items():
        logger.info(f"  {stype}: {len(runs)} runs")

    # =========================================================================
    # Output directory
    # =========================================================================

    out_dir = os.path.join(SCRATCH_DIR, 'output', 'rest_03_projected', parc, sub_id)
    if args.vt is not None:
        out_dir = os.path.join(out_dir, f'vt{args.vt}')
    os.makedirs(out_dir, exist_ok=True)

    # =========================================================================
    # Project each rest run and collect data for diagnostic
    # =========================================================================
    ...
    diag_by_type = {stype: _new_stats_bucket(n_parcels) for stype in REST_PREFIXES}
    rest_run_ids = {}   # stimulus_type -> list of run_ids
    projected_count = 0
    ...
    for stype, runs in rest_runs.items():
        ...
        for run_id, fpath in runs:
            ...
            if X.shape[1] != pca.n_features_in_:
                logger.error(
                    ...
                    "Parcellation mismatch between rest data and Friends PCA model."
                )
                sys.exit(1)
        ...
        rest_run_ids[stype] = run_id_list

    logger.info(f"Projected {projected_count} rest runs to {n_pcs} PCs")

    # =========================================================================
    # Save rest run ID grouping
    # =========================================================================

    run_ids_path = os.path.join(out_dir, 'rest_run_ids.json')
    with open(run_ids_path, 'w') as f:
        json.dump(rest_run_ids, f, indent=2)

    # =========================================================================
    # PCA transfer diagnostic (from online-accumulated statistics)
    # =========================================================================

    logger.info(f"Computing PCA transfer diagnostic on {diag_overall['count']} total rest TRs")
    ...
    print(f"\n{'='*60}")
    print(f"PCA TRANSFER DIAGNOSTIC (Resting State)")
    print(f"{'='*60}")
    print(f"Subject:            {sub_id}")
    print(f"Parcellation:       {parc}")
    print(f"Rest runs:          {projected_count}")
    print(f"PCs used:           {n_pcs}")
    print(f"Friends R² (n_pcs): {diagnostic['friends_r2_n_pcs']:.4f}")
    print(f"Rest R² (n_pcs):    {diagnostic['rest_r2_n_pcs']:.4f}")
    print(f"Rest R² (mean-corr):{diagnostic['rest_r2_n_pcs_mean_corrected']:.4f}")
    ...
    if diagnostic['flag_low_variance']:
        print(f"*** WARNING: Rest R² < 0.70 - PCA subspace may not capture rest variance ***")
    print(f"\nPer-type R² (n_pcs):")
    ...
    print(f"\nParcel mean shift (rest vs Friends):")
    ...
    try:
        ...
        def _render_pca_diagnostic_figure(
            ...
            Args:
                ...
                scope_label: label for suptitle (e.g., "Overall" or "Restingstate")
            ...
            fig.suptitle(
                f'PCA Transfer: Friends → Rest - {scope_label} ({sub_id}{vt_str})',
                fontsize=12, y=0.98)
        ...
        _render_pca_diagnostic_figure(
            ...
            overall_r2=diagnostic['rest_r2_n_pcs'],
            scope_label='Overall',
            r2_bar_items=[
                ('Friends R²', diagnostic['friends_r2_n_pcs'], '#4682B4'),
                ('Rest R²', diagnostic['rest_r2_n_pcs'], '#CD3E4E'),
                ('Rest R² (mc)', diagnostic['rest_r2_n_pcs_mean_corrected'], '#E69422'),
            ],
