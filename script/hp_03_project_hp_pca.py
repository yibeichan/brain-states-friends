#!/usr/bin/env python3
"""
hp_03_project_hp_pca.py - Project Harry Potter parcel time series through Friends-trained PCA.

Loads the PCA model fitted on Friends training data (from 04_combined_hdphmm final/)
and projects each HP run's parcel time series into the same PCA space. Computes a
...
Prerequisites:
    - 00_postproc.py completed for harrypotter (cleaned CIFTIs)
    - 02_extract_parcel_ts.py completed for harrypotter episodes
    - 04_combined_hdphmm.py (mode: select) completed for this subject (Friends model)

Outputs:
    {SCRATCH_DIR}/output/hp_03_projected/{parcellation}/{sub_id}/[vt{VT}/]
        {run_id}.npy                  - PCA-projected HP run (n_trs, n_pcs)
        hp_run_ids.json               - Run IDs grouped by stimulus type
...
# Harry Potter stimulus type prefix
HP_PREFIXES = {
...
def get_stimulus_type(run_id):
    ...
    Args:
        run_id: Full BIDS run ID (e.g., 'sub-01_task-harrypotter_run-1_space-fsLR_den-91k')
                or just the task portion (e.g., 'harrypotter_run-1')

    Returns:
        Stimulus type string ('harrypotter') or None
    """
...
def discover_hp_runs(parcel_ts_dir):
    """Discover Harry Potter run files in the parcel time series directory.

    Finds all .npy files with harrypotter task prefix, excluding Friends episodes
    (task-s*) and Movie10 runs.
    ...
    hp_runs = {stype: [] for stype in HP_PREFIXES}
    ...
    for fpath in all_files:
        ...
        if stype is not None:
            hp_runs[stype].append((run_id, fpath))
...
def compute_pca_transfer_diagnostic_from_stats(pca, n_pcs, stats):
    ...
    # Per-parcel mean shift: HP parcel means vs Friends training means
    hp_parcel_means = ov['parcel_sum'] / ov['count'] if ov['count'] > 0 else np.zeros(n_parcels)
    mean_shift = hp_parcel_means - pca.mean_
    ...
    if ov['count'] > 0:
        ...
        sst_mc = sum_x_sq - ov['count'] * np.dot(hp_parcel_means, hp_parcel_means)

        shift = pca.mean_ - hp_parcel_means
    ...
    diagnostic = {
        ...
        'hp_r2_n_pcs': float(r_squared),
        'hp_r2_n_pcs_mean_corrected': float(r_squared_mc),
        'hp_r2_full_pca': float(r_squared_full),
        'friends_r2_n_pcs': friends_r2_n_pcs,
        'friends_r2_full_pca': friends_r2_full,
        'n_pcs': n_pcs,
        'n_components_total': pca.n_components_,
        'n_hp_trs': int(ov['count']),
        'n_parcels': n_parcels,
...
def parse_args():
    parser = argparse.ArgumentParser(
        description='Project Harry Potter parcel time series through Friends-trained PCA.',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python script/hp_03_project_hp_pca.py --sub_id sub-01
  python script/hp_03_project_hp_pca.py --sub_id sub-01 --parcellation atlas-4S456Parcels
        """
    )
...
def main():
    ...
    if not os.path.isdir(parcel_ts_dir):
        ...
        logger.error("Run hp_00_postproc.sh and hp_02_extract_parcel_ts.sh first.")
        sys.exit(1)
    ...
    # =========================================================================
    # Discover and group HP runs
    # =========================================================================

    hp_runs = discover_hp_runs(parcel_ts_dir)

    total_runs = sum(len(runs) for runs in hp_runs.values())
    if total_runs == 0:
        logger.error(f"No HP runs found in {parcel_ts_dir}")
        logger.error("Expected files matching *task-harrypotter*_parcel_avg.npy")
        sys.exit(1)

    logger.info(f"Discovered {total_runs} HP runs:")
    for stype, runs in hp_runs.items():
    ...
    out_dir = os.path.join(SCRATCH_DIR, 'output', 'hp_03_projected', parc, sub_id)
    if args.vt is not None:
        out_dir = os.path.join(out_dir, f'vt{args.vt}')
    os.makedirs(out_dir, exist_ok=True)

    # =========================================================================
    # Project each HP run and collect data for diagnostic
    # =========================================================================
    ...
    diag_by_type = {stype: _new_stats_bucket(n_parcels) for stype in HP_PREFIXES}
    hp_run_ids = {}   # stimulus_type -> list of run_ids
    projected_count = 0
    ...
    for stype, runs in hp_runs.items():
        ...
        for run_id, fpath in runs:
            ...
            if X.shape[1] != pca.n_features_in_:
                logger.error(
                    ...
                    "Parcellation mismatch between HP data and Friends PCA model."
                )
                sys.exit(1)
        ...
        hp_run_ids[stype] = run_id_list

    logger.info(f"Projected {projected_count} HP runs to {n_pcs} PCs")

    # =========================================================================
    # Save HP run ID grouping
    # =========================================================================

    run_ids_path = os.path.join(out_dir, 'hp_run_ids.json')
    with open(run_ids_path, 'w') as f:
        json.dump(hp_run_ids, f, indent=2)

    # =========================================================================
    # PCA transfer diagnostic (from online-accumulated statistics)
    # =========================================================================

    logger.info(f"Computing PCA transfer diagnostic on {diag_overall['count']} total HP TRs")
    ...
    print(f"\n{'='*60}")
    print(f"PCA TRANSFER DIAGNOSTIC (Harry Potter Reading)")
    print(f"{'='*60}")
    print(f"Subject:            {sub_id}")
    print(f"Parcellation:       {parc}")
    print(f"HP runs:            {projected_count}")
    print(f"PCs used:           {n_pcs}")
    print(f"Friends R² (n_pcs): {diagnostic['friends_r2_n_pcs']:.4f}")
    print(f"HP R² (n_pcs):      {diagnostic['hp_r2_n_pcs']:.4f}")
    print(f"HP R² (mean-corr):  {diagnostic['hp_r2_n_pcs_mean_corrected']:.4f}")
    ...
    print(f"\nParcel mean shift (HP vs Friends):")
    ...
    try:
        ...
        def _render_pca_diagnostic_figure(
            ...
            Args:
                ...
                scope_label: label for suptitle (e.g., "Overall" or "Harrypotter")
        ...
        _render_pca_diagnostic_figure(
            ...
            overall_r2=diagnostic['hp_r2_n_pcs'],
            scope_label='Overall',
            r2_bar_items=[
                ('Friends R²', diagnostic['friends_r2_n_pcs'], '#4682B4'),
                ('HP R²', diagnostic['hp_r2_n_pcs'], '#CD3E4E'),
                ('HP R² (mc)', diagnostic['hp_r2_n_pcs_mean_corrected'], '#E69422'),
            ],
