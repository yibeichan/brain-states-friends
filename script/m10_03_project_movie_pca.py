#!/usr/bin/env python3
"""
m10_03_project_movie_pca.py - Project movie10 parcel time series through Friends-trained PCA.

Loads the PCA model fitted on Friends training data (from 04_combined_hdphmm final/)
and projects each movie run's parcel time series into the same PCA space. Computes a
PCA transfer diagnostic (variance explained by Friends PCA on movie data).

Prerequisites:
    - 00_postproc.py completed for movie10 (cleaned CIFTIs)
    - 02_extract_parcel_ts.py completed for movie10 episodes
    - 04_combined_hdphmm.py (mode: select) completed for this subject (Friends model)

Outputs:
    {SCRATCH_DIR}/output/m10_03_projected/{parcellation}/{sub_id}/[vt{VT}/]
        {run_id}.npy                  - PCA-projected movie run (n_trs, n_pcs)
        movie_run_ids.json            - Run IDs grouped by movie type
...
# Movie type prefixes for grouping
MOVIE_PREFIXES = {
    'bourne': 'bourne',
    'wolf': 'wolf',
    'figures': 'figures',
...
def get_movie_type(run_id):
    ...
    Args:
        run_id: Full BIDS run ID (e.g., 'sub-01_ses-001_task-bourne01_space-fsLR_den-91k')
                or just the task portion (e.g., 'bourne01')

    Returns:
        Movie type string ('bourne', 'wolf', 'figures', 'life') or None
    """
    # Extract task label from full BIDS run_id
...
def discover_movie_runs(parcel_ts_dir):
    """Discover movie run files in the parcel time series directory.

    Finds all .npy files with movie task prefixes (bourne, wolf, figures, life),
    excluding Friends episodes (task-s*).

    Args:
        parcel_ts_dir: Path to 02_parcel_ts_avg/{parc}/{sub}/

    Returns:
        dict: movie_type -> list of (run_id, filepath) tuples
    """
    all_files = sorted(glob.glob(os.path.join(parcel_ts_dir, '*_parcel_avg.npy')))

    movie_runs = {mtype: [] for mtype in MOVIE_PREFIXES}

    for fpath in all_files:
        fname = os.path.basename(fpath)
        run_id = fname.replace('_parcel_avg.npy', '')

        mtype = get_movie_type(run_id)
        if mtype is not None:
            movie_runs[mtype].append((run_id, fpath))
...
def compute_pca_transfer_diagnostic_from_stats(pca, n_pcs, stats):
    ...
    Avoids holding all movie data in memory simultaneously by using
    ...
    for mtype, ts in stats['by_type'].items():
        if ts['count'] > 0 and ts['sst'] > 0:
            ...
            # Per-network R² within this movie type
            if 'by_network' in ts:
                type_net_r2 = {}
                for net in NETWORK_ORDER:
                    nb = ts['by_network'].get(net)
                    if nb and nb['sst'] > 0:
                        type_net_r2[net] = {
                            'r2_n_pcs': float(1.0 - nb['sse_n'] / nb['sst']),
                            'n_parcels': int(len(ts['_network_indices'].get(net, []))),
                        }
                if type_net_r2:
                    r2_by_type[mtype]['r2_by_network'] = type_net_r2

    # Per-parcel mean shift: movie parcel means vs Friends training means
    movie_parcel_means = ov['parcel_sum'] / ov['count'] if ov['count'] > 0 else np.zeros(n_parcels)
    mean_shift = movie_parcel_means - pca.mean_
    ...
    # Derivation: SST_mc = Σ||X - μ_data||², SSE_mc = SSE_n - n·||δ||²
    # where δ = (μ_friends - μ_data) projected into PCA null space.
    if ov['count'] > 0:
        sum_x_sq = ov['sst'] + 2.0 * np.dot(pca.mean_, ov['parcel_sum']) \
            - ov['count'] * np.dot(pca.mean_, pca.mean_)
        sst_mc = sum_x_sq - ov['count'] * np.dot(movie_parcel_means, movie_parcel_means)

        shift = pca.mean_ - movie_parcel_means
    ...
    diagnostic = {
        'movie_r2_n_pcs': float(r_squared),
        'movie_r2_n_pcs_mean_corrected': float(r_squared_mc),
        'movie_r2_full_pca': float(r_squared_full),
        'friends_r2_n_pcs': friends_r2_n_pcs,
        'friends_r2_full_pca': friends_r2_full,
        'n_pcs': n_pcs,
        'n_components_total': pca.n_components_,
        'n_movie_trs': int(ov['count']),
        'n_parcels': n_parcels,
...
def parse_args():
    parser = argparse.ArgumentParser(
        description='Project movie10 parcel time series through Friends-trained PCA.',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python script/m10_03_project_movie_pca.py --sub_id sub-01
  python script/m10_03_project_movie_pca.py --sub_id sub-01 --parcellation atlas-4S456Parcels
        """
    )
...
def main():
    ...
    if not os.path.isdir(parcel_ts_dir):
        ...
        logger.error("Run m10_00_postproc.sh and m10_02_extract_parcel_ts.sh first.")
        sys.exit(1)
    ...
    # =========================================================================
    # Discover and group movie runs
    # =========================================================================

    movie_runs = discover_movie_runs(parcel_ts_dir)

    total_runs = sum(len(runs) for runs in movie_runs.values())
    if total_runs == 0:
        logger.error(f"No movie runs found in {parcel_ts_dir}")
        logger.error("Expected files matching *task-{bourne,wolf,figures,life}*_parcel_avg.npy")
        sys.exit(1)

    logger.info(f"Discovered {total_runs} movie runs:")
    for mtype, runs in movie_runs.items():
        logger.info(f"  {mtype}: {len(runs)} runs")

    # =========================================================================
    # Output directory
    # =========================================================================

    out_dir = os.path.join(SCRATCH_DIR, 'output', 'm10_03_projected', parc, sub_id)
    if args.vt is not None:
        out_dir = os.path.join(out_dir, f'vt{args.vt}')
    os.makedirs(out_dir, exist_ok=True)

    # =========================================================================
    # Project each movie run and collect data for diagnostic
    # =========================================================================
    ...
    diag_by_type = {mtype: _new_stats_bucket(n_parcels, network_indices) for mtype in MOVIE_PREFIXES}
    movie_run_ids = {}   # movie_type -> list of run_ids
    projected_count = 0
    ...
    for mtype, runs in movie_runs.items():
        ...
        for run_id, fpath in runs:
            ...
            if X.shape[1] != pca.n_features_in_:
                logger.error(
                    ...
                    "Parcellation mismatch between movie data and Friends PCA model."
                )
                sys.exit(1)
            logger.info(f"  {run_id}: shape {X.shape}")

            # Accumulate R² statistics online (avoids large concatenation)
            # First call returns the truncated projection for saving
            X_proj = _accumulate_r2_stats(pca, n_pcs, X, diag_overall)
            _accumulate_r2_stats(pca, n_pcs, X, diag_by_type[mtype])
        ...
        movie_run_ids[mtype] = run_id_list

    logger.info(f"Projected {projected_count} movie runs to {n_pcs} PCs")

    # =========================================================================
    # Save movie run ID grouping
    # =========================================================================

    run_ids_path = os.path.join(out_dir, 'movie_run_ids.json')
    with open(run_ids_path, 'w') as f:
        json.dump(movie_run_ids, f, indent=2)

    # =========================================================================
    # PCA transfer diagnostic (from online-accumulated statistics)
    # =========================================================================

    logger.info(f"Computing PCA transfer diagnostic on {diag_overall['count']} total movie TRs")
    ...
    print(f"\n{'='*60}")
    print(f"PCA TRANSFER DIAGNOSTIC")
    print(f"{'='*60}")
    print(f"Subject:            {sub_id}")
    print(f"Parcellation:       {parc}")
    print(f"Movie runs:         {projected_count}")
    print(f"PCs used:           {n_pcs}")
    print(f"Friends R² (n_pcs): {diagnostic['friends_r2_n_pcs']:.4f}")
    print(f"Movie R² (n_pcs):   {diagnostic['movie_r2_n_pcs']:.4f}")
    print(f"Movie R² (mean-corr):{diagnostic['movie_r2_n_pcs_mean_corrected']:.4f}")
    ...
    if diagnostic['flag_low_variance']:
        print(f"*** WARNING: Movie R² < 0.70 — PCA subspace may not capture movie variance ***")
    print(f"\nPer-movie-type R² (n_pcs):")
    for mtype, r2_info in diagnostic['r2_by_movie_type'].items():
        print(f"  {mtype:10s}: {r2_info['r2_n_pcs']:.4f}  ({r2_info['n_trs']} TRs)")
    ...
    print(f"\nParcel mean shift (movie vs Friends):")
    ...
    try:
        ...
        def _render_pca_diagnostic_figure(
            ...
            Args:
                r2_by_network: dict net -> {r2_n_pcs, n_parcels} for brain coloring + bar chart
                overall_r2: scalar R² for dashed reference line on bar chart
                scope_label: label for suptitle (e.g., "Overall" or "Bourne")
                r2_bar_items: list of (label, value, color) for Row 2 comparison bars
            ...
            fig.suptitle(
                f'PCA Transfer: Friends → Movie10 — {scope_label} ({sub_id}{vt_str})',
                fontsize=12, y=0.98)
        ...
        _render_pca_diagnostic_figure(
            ...
            overall_r2=diagnostic['movie_r2_n_pcs'],
            scope_label='Overall',
            r2_bar_items=[
                ('Friends R²', diagnostic['friends_r2_n_pcs'], '#4682B4'),
                ('Movie R²', diagnostic['movie_r2_n_pcs'], '#00760E'),
                ('Movie R² (mc)', diagnostic['movie_r2_n_pcs_mean_corrected'], '#E69422'),
            ],
            out_path=os.path.join(out_dir, 'pca_transfer_diagnostic.png'),
        )

        # -- Per-movie-type diagnostic figures --
        for mtype, type_info in diagnostic['r2_by_movie_type'].items():
            if 'r2_by_network' not in type_info:
                continue
            _render_pca_diagnostic_figure(
                r2_by_network=type_info['r2_by_network'],
                overall_r2=type_info['r2_n_pcs'],
                scope_label=mtype.capitalize(),
                r2_bar_items=[
                    ('Friends R²', diagnostic['friends_r2_n_pcs'], '#4682B4'),
                    ('Movie (all)', diagnostic['movie_r2_n_pcs'], '#00760E'),
                    (f'{mtype.capitalize()} R²', type_info['r2_n_pcs'], '#E69422'),
                ],
                out_path=os.path.join(out_dir, f'pca_transfer_diagnostic_{mtype}.png'),
            )
