...
Harry Potter is a unimodal reading stimulus (word-by-word RSVP at 2Hz). This tests whether
Friends-trained brain states produce meaningful state sequences from reading data.

Prerequisites:
    - hp_03_project_hp_pca.py completed (projected HP data + hp_run_ids.json)
    - 04_combined_hdphmm.py (mode: select) completed (Friends model)

Outputs:
    {SCRATCH_DIR}/output/hp_04_decoded/{parcellation}/{sub_id}/
        decoded_states.pkl         - dict: run_id -> np.array(n_trs,) state indices
        fractional_occupancy.pkl   - dict: run_id -> np.array(n_states,)
        hp_ll_summary.json         - Per-run LL, overall LL, baselines
...
def parse_args():
    parser = argparse.ArgumentParser(
        description='Score and decode Harry Potter data with Friends-trained HMM.',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python script/hp_04_score_and_decode.py --sub_id sub-01
  python script/hp_04_score_and_decode.py --sub_id sub-01 --parcellation atlas-4S456Parcels
        """
    )
...
def main():
    ...
    if args.vt is not None:
        out_dir = os.path.join(SCRATCH_DIR, 'output', 'hp_04_decoded', parc, sub_id, f'vt{args.vt}')
    else:
        out_dir = os.path.join(SCRATCH_DIR, 'output', 'hp_04_decoded', parc, sub_id)
    os.makedirs(out_dir, exist_ok=True)

    # =========================================================================
    # Score and decode each HP run
    # =========================================================================
    ...
    if total_runs_decoded == 0:
        logger.error("No HP runs were decoded - check hp_03 outputs")
        sys.exit(1)
    ...
    # Overall HP LL (weighted average by n_trs)
    ...
    # Per-type LL (HP has only one type, but keep structure for consistency)
    per_type_ll = {}
    for stype, run_ids in hp_run_ids.items():
    ...
    ll_summary = {
        ...
        'hp_overall_ll_per_sample': float(overall_hp_ll),
        'hp_ll_per_run_mean': run_ll_mean,
        'hp_ll_per_run_std': run_ll_std,
        'hp_ll_per_run_se': run_ll_se,
        'hp_total_trs': int(total_trs),
        'hp_total_runs': total_runs_decoded,
        ...
        'll_gap_friends_minus_hp': float(friends_test_ll - overall_hp_ll),
        'hp_above_baseline': bool(overall_hp_ll > baseline_ll),
    ...
    # decoded_states with 08c feature files directly. Legacy long-BIDS
    # copies are retained during the phase-1 migration so 07b/07c/hp_05
    # keep working until they are updated.
    long_to_short = {
    ...
    print(f"\n{'='*60}")
    print(f"HARRY POTTER SCORE & DECODE SUMMARY")
    print(f"{'='*60}")
    print(f"Subject:                {sub_id}")
    print(f"Parcellation:           {parc}")
    print(f"HP runs decoded:        {total_runs_decoded}")
    print(f"Total HP TRs:           {total_trs}")
    print(f"Friends test LL/sample: {friends_test_ll:.4f}")
    print(f"HP overall LL/sample:   {overall_hp_ll:.4f}")
    print(f"Baseline LL/sample:     {baseline_ll:.4f}")
    print(f"LL gap (Friends-HP):    {friends_test_ll - overall_hp_ll:.4f}")
    print(f"HP > baseline:          {overall_hp_ll > baseline_ll}")
    print(f"")
    ...
    try:
        ...
        ax_ll.axhline(overall_hp_ll, color=NETWORK_COLORS['Default'], ls='--', lw=1.0,
                      label=f'HP mean LL ({overall_hp_ll:.2f})')
        ...
        gap = friends_test_ll - overall_hp_ll
        ...
        fig.suptitle(f'HP Score & Decode ({sub_id}{vt_str})', fontsize=12)
        fig.tight_layout()
