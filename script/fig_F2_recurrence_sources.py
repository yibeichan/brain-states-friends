...
| D | Surface exemplars per category (sub-01): cortical + subcortical mean-activity maps with shared per-region colorbars. No temporal inset - the temporal screens do not share a single legible axis (only run-onset separates on within-run position; drift and content-eligible have no simple within-figure signature). | brain map | 04 state_means_parcel.npy + yabplot | fig2_D_*.{pdf,png,svg} (multi-file; SVG brain-map files embed raster renders) |
...
def config(Path, os):
    ...
    # Shared legibility scheme for panels A and B (7-inch width, larger fonts).
    FS_TICK = 8
    FS_LABEL = 9
    FS_ANNOT = 8
    FIG_W = 7.0
    return (SCRATCH_DIR, PARCELLATION, VT, SUBJECTS, FLAGS_DIR, MODEL_DIR, OUT_F2,
            FS_TICK, FS_LABEL, FS_ANNOT, FIG_W)
...
def panel_A_category_bars(
    ...
    FS_TICK, FS_LABEL, FS_ANNOT, FIG_W, plt, np,
):
    """Panel A - per-subject category-count stacked bars (one combined figure)
    plus a standalone category legend.
    ...
    segment color = category. The legend is decomposed into a separate file.
    """
    ...
    _row_h = 0.64
    _fig, _ax = plt.subplots(figsize=(FIG_W, 3.3))
    ...
    for _j, _sub in enumerate(SUBJECTS):
        ...
        for _cat in TAXONOMY_ORDER:
            ...
            _ax.text(_x_cursor + _n / 2, _j, str(_n), ha="center",
                     va="center", fontsize=FS_ANNOT, color=_TEXT_ON[_cat], zorder=3)
            _x_cursor += _n

    _ax.set_yticks(np.arange(len(SUBJECTS)))
    _ax.set_yticklabels(SUBJECTS, fontsize=FS_LABEL)
    _ax.tick_params(axis="y", length=0)
    _ax.set_ylim(-0.6, len(SUBJECTS) - 0.4)
    _ax.set_xlim(0, 50)
    _ax.set_xticks([0, 10, 20, 30, 40, 50])
    _ax.set_xlabel("Number of latent states", fontsize=FS_LABEL)
    ...
    _fig.subplots_adjust(left=0.08, right=0.98, bottom=0.14, top=0.97)
    ...
    # Standalone category legend (the shared key), one horizontal row.
    ...
    _figl, _axl = plt.subplots(figsize=(FIG_W, 0.4))
    _axl.axis("off")
    _axl.legend(handles=_handles, loc="center", ncol=5, frameon=False,
                fontsize=FS_ANNOT, handlelength=1.1, handletextpad=0.4,
                columnspacing=1.0)
    _steml = OUT_F2 / "fig2_A_legend_categories"
    _figl.savefig(f"{_steml}.png", bbox_inches="tight", pad_inches=0.02, dpi=300)
    _figl.savefig(f"{_steml}.svg", bbox_inches="tight", pad_inches=0.02)
    plt.close(_figl)
    print(f"saved: {_steml.name}.png (+ .svg)")
    return
...
def panel_B_category_network_sankey(
    ...
    TAXONOMY_ORDER, TAXONOMY_COLORS, TAXONOMY_MAP,
    NETWORK_ORDER, NETWORK_COLORS, display_network,
    FS_ANNOT, FIG_W, plt, np,
):
    ...
    from utils.plot_style import load_parcel_networks as _load_parcel_networks
    from utils.network_participation import network_composition_for_state

    _K_TOP = 3
    _parcel_nets = np.array(_load_parcel_networks(PARCELLATION))
    ...
    _fig, _ax = plt.subplots(figsize=(FIG_W, 4.8))
    _node_w = 0.018
    ...
    # Left category names and right cortical-network names get a slightly larger
    # size; the small-flow subcortical names stay at the base size.
    _fs_emph = FS_ANNOT + 2
    _fs_subc = FS_ANNOT
    ...
    for _i in range(len(TAXONOMY_ORDER)):
        ...
        _ax.text(
            ...
            ha="right", va="center", fontsize=_fs_emph,
    ...
    for _j in range(len(NETWORK_ORDER)):
        ...
        _fs_net = _fs_emph if NETWORK_ORDER[_j] in _CORTICAL else _fs_subc
        _ax.text(
            1.015, _net_top[_j] + _net_y_off + _node_visual_h[_j] / 2,
            f"{display_network(NETWORK_ORDER[_j])} ({_pct:.1f}%)",
            ha="left", va="center", fontsize=_fs_net,
            color=_LABEL_DARKEN.get(NETWORK_ORDER[_j], NETWORK_COLORS[NETWORK_ORDER[_j]]),
        )

    # Wider x-margins so the larger left/right labels do not clip.
    _ax.set_xlim(-0.32, 1.32)
...
def panel_C_network_participation(
    ...
    """Panel C - per-state network-participation pie mosaic (content-eligible).

    Main panel is sub-01: one mini-pie per content-eligible state, each pie the
    state's 13-network composition (same colors as the Panel B Sankey), sorted
    by dominant network. This is the per-state complement to B's aggregate
    flow: a Sankey cannot tell "every state is multi-network" from "each state
    is single-network but different states pick different networks". The mosaic
    shows the former - no pie is a single solid wedge. Other subjects replicate
    into fig2/supp/<sub>/. Metrics CSV/JSON (all 6 subjects) provide the caption
    statistics. Descriptive composition, not a claim that canonical networks are
    the discovered state units.
    """
    from utils.plot_style import load_parcel_networks as _load_parcel_networks
    ...
    _main = "sub-01"
    plot_network_pie_mosaic(_metrics, _main, NETWORK_ORDER, NETWORK_COLORS,
                            OUT_F2 / f"fig2_C_{_main}_network_pies")
    plot_network_legend(NETWORK_ORDER, NETWORK_COLORS,
                        OUT_F2 / "fig2_C_legend_networks",
                        display_fn=display_network, ncol=2, fig_w=2.6)
    for _sub in [_s for _s in SUBJECTS if _s != _main]:
        _supp = OUT_F2 / "supp" / _sub
        _supp.mkdir(parents=True, exist_ok=True)
        plot_network_pie_mosaic(_metrics, _sub, NETWORK_ORDER, NETWORK_COLORS,
                                _supp / f"fig2_C_{_sub}_network_pies")

    print(f"saved: fig2_C_{_main}_network_pies + supp mosaics + legend")
