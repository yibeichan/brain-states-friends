...
#
# Prerequisites:
#   - hp_03_project_hp_pca.py completed (projected HP data)
#   - 04_combined_hdphmm.py (mode: select) completed (Friends model)
#
# Usage:
#   sbatch script/hp_04_score_and_decode.sh
#   sbatch --array=0 script/hp_04_score_and_decode.sh              # sub-01 only
...
echo "=========================================="
echo "Harry Potter Score & Decode"
echo "=========================================="
...
uv run --project "${PROJECT_DIR}" --no-sync python "${PROJECT_DIR}/script/hp_04_score_and_decode.py" \
    --sub_id "$SUBJECT_ID" \
