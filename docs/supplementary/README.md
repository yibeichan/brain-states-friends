...
These panels should be read as a statement about the shared subspace, not as validation of individual state maps. Spatial convergence depends on granularity: it sits at or near the rotation null for coarse, HMM-independent decompositions (K = 15 is null in all six subjects, K = 25 in four of six) and emerges only as the ICA dimensionality approaches each subject's K_active, varying substantially across subjects. A random orthonormal rotation of the same PCA subspace already matches the HMM state-mean maps about as well as ICA's independence-maximizing rotation, so only the agreement exceeding that null is informative. Matches to low-confidence, unused, or rare states indicate that those states' mean maps are recoverable within the shared subspace. They are not independent biological or content validation.
