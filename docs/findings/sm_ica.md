# Findings: ICA convergent-validity supplement (`sm/ica-states`)

Findings for the ICA-vs-HMM convergent-validity supplement. Code: `script/sm_ica_states.py` (+`.sh`), `script/sm_ica_category_table.py`; companion figure `script/fig_sm_ica_matching.py`.
...
- **Dimensionality sweep:** K ∈ {15, 25, 35} (primary low-dim) + K_active + a **K=41–47 sensitivity grid** (matches each subject's K_active range 37–42; n_components ≤ n_pcs for all). Reporting the full sweep is the design's circularity control - no single K is privileged.
...
- **Per-category breakdown** (`sm_ica_category_table.py`): the K_active "all"-set matched pairs labeled by HMM-state `summary_category` (05e state_flags).

## Results

### Tier 1 - spatial convergence is granularity-dependent, subject-variable
...
- Coarse ICA matches the HMM **no better than a random rotation of the shared subspace** at the HMM-independent low-dim settings: K=15 is null for all 6; K=25 is null for four subjects, with sub-02 at 8/25 and a single-pair blip in sub-06. This low-dim sweep, the design's primary, is null-to-weak.
- Convergence **emerges at K=35 and strengthens toward K≈K_active** (41–47): strong in sub-01/sub-05, moderate in sub-02/sub-06, weak in sub-04, **absent in sub-03** (zero at every K). The convergence regime (K≈K_active) is the one reviewers flagged as circular, so it is reported as a sensitivity, not a headline.
...
### Per-category - convergence is taxonomy-agnostic
...
**Caveat (binding for the supplement):** matches to low-confidence / unused / rare states indicate recoverability of those states' mean maps within the shared subspace - **not** independent biological or content validation. The state quality flags stand. The substantive convergent-validity signal rests on the content-eligible (and, more weakly, run-onset) repertoire.

### Tier 2 - temporal correspondence broad but conditional
...
## Honest-qualified conclusion (agreed framing)

Spatial convergence between ICA and the HMM is **granularity-dependent** (null at coarse, HMM-independent K; emerges only when ICA dimensionality approaches the HMM's K_active), **subject-variable** (strong in 2/6, moderate in 2/6, weak in 1, absent in 1), and **taxonomy-agnostic** (not specific to content-eligible states; artifact-category matches are subspace recoverability, not validation). Temporal correspondence is broad but rests on a weaker null and is conditional on the spatial match. The supplement reports this as qualified supporting evidence; it does not claim that an independent method cleanly rediscovers the HMM repertoire.
...
`script/fig_sm_ica_matching.py`. Panel A: K-sweep convergence heatmap (FDR-surviving eligible fraction, 6 subjects x K). Panel B: per-state matched |r| at K_active, x = subject, colour = taxonomy category, per-subject median tick. Panel B is a per-state strip showing the full per-subject distribution and category mix without collapsing to means.
